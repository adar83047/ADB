...
export default function BurgersScreen() {
  ...
  useEffect(() => {
    fetch('http://localhost:3000/api/burgers')
      ...
      .catch(() => {
        setBurgers([
          { id: 1, name: 'Classic Burger', image: require('../../../assets/images/burger.png'), price: 400 },
          { id: 2, name: 'Cheese Burger', image: require('../../../assets/images/burger.png'), price: 450 },
        ]);
      });
  }, []);
        ...
        <ScrollView contentContainerStyle={{ padding: 16 }}>
          {burgers.map((burger) => (
            <View key={burger.id} style={styles.card}>
              <Image source={burger.image} style={styles.pizzaImg} />
              <Text style={styles.pizzaName}>{burger.name}</Text>
              <Text style={styles.pizzaPrice}>$ {burger.price}</Text>
              <TouchableOpacity style={styles.addBtn} onPress={() => addToCart(burger)}>
